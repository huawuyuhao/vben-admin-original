...
const { isCollecting, isUsing, toggleCollect, useNow } = useFavoriteActions();
...
/** 标签列表 */
const tags = computed(() => splitFavoriteTags(props.item.tags));

/** 价格文案 */
const priceText = computed(
  () =>
    formatFavoritePrice(props.item.price) ||
    $t('page.service.product.pricePending'),
);

/** 绿电占比文案 */
const greenText = computed(() =>
  formatFavoriteGreenPowerRatio(props.item.greenPowerRatio),
);
...
<template>
  ...
  >
    <div
      ...
      :class="{ 'product-card__cover--empty': !hasFavoriteImage(item.imageUrl) }"
    >
      <el-image
        v-if="hasFavoriteImage(item.imageUrl)"
