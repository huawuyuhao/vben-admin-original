...
defineOptions({ name: 'AdminCaseFilterBar' });
...
/** 是否草稿（与父级 v-model:isDraft 同步，走接口） */
const isDraft = defineModel<CaseDraftFilter>('isDraft', { default: '' });
  ...
  <el-card class="case-filter" shadow="never">
    <el-form class="case-filter__form" @submit.prevent>
      ...
      <p class="case-filter__scope-hint">
        ...
        <span>{{ $t('page.monitoring.content.case.filter.scopeHint') }}</span>
      </p>
